...
function kpClass(k) {
  if (progress.value.mastered[k.id]) return 'done'
  if (progress.value.read[k.id]) return ''
  return ''
}
...
<template>
  <div v-if="module">
    ...
    <div class="card page-head stagger" :style="{ borderTop: '3px solid ' + module.color }">
      <div class="meta-row">
      ...
      <p class="summary">{{ module.subtitle }}</p>
    </div>

    <div v-for="(c, ci) in module.chapters" :key="c.id" class="card chapter-item stagger" :style="{ animationDelay: ci * 60 + 'ms' }">
      <div class="chapter-head" :class="{ open: isOpen(c.id) }" @click="toggle(c.id)">
        ...
        <span style="color: var(--faint); font-size: 12px">{{ c.kps.length }} 个知识点</span>
      ...
      <div class="chapter-body" :class="{ open: isOpen(c.id) }">
        <div>
          <div class="chapter-inner">
            <div class="obj-list">
              <div class="section-title" style="font-size: 12.5px">🎯 学习目标</div>
              <ul style="padding-left: 20px; color: var(--muted); font-size: 13px; line-height: 1.9">
                <li v-for="(o, i) in c.objectives" :key="i">{{ o }}</li>
              </ul>
            </div>
            <div class="section-title" style="font-size: 12.5px">🧩 知识点</div>
            <div class="kp-chips">
              <span
                v-for="k in c.kps"
                :key="k.id"
              ...
              >
                ...
                <span v-if="k.hasDetail" style="color: var(--faint); font-size: 10px">📝</span>
              </span>
            </div>
            <div class="section-title" style="font-size: 12.5px">🏁 关卡测验（覆盖本章全部知识点）</div>
